...
パターンを転写する工程\
ArF/KrF/EUVなどの光でレジスト（感光材料）に回路パターンを焼き付け、現像する\
ArF・KrFはDUV露光のレーザー光源 波長が短いほど微細なパターンを描ける
...
* - [Nikon]{.jp} / [Canon]{.jp}
  - DUV露光装置
  - 成熟ノード（回路の微細度の世代 例:3nm/2nm）で競合\
    ArF液浸/KrF等 各社約5%程度 EUVは非供給
:::
...
光がマスクとレンズのガラスを透過して結像する（大気中・ArFは液浸）

:::{image} _static/images/litho-duv-transmissive.svg
:alt: DUV露光の光路 光がマスクとレンズを透過してウェハーに縮小投影される
:width: 56%
:align: center
:::

[出典: 光路は一般的なDUV露光の自作図/2024年頃\
詳細は docs「前工程」ページ]{.source}

### EUVの光路（反射・ミラー）

EUV(13.5nm)は吸収されレンズを使えず 真空中の反射ミラーで結像する

:::{image} _static/images/litho-euv-reflective.svg
:alt: EUV露光の光路 反射マスクとミラーで反射を繰り返しウェハーに縮小投影される
:width: 56%
:align: center
:::

[出典: 光路は一般的なEUV露光の自作図/2024年頃\
詳細は docs「前工程」ページ]{.source}
...
シリコン結晶に不純物（微量の別元素）を高速で打ち込み、電気の流れやすさを作り分ける工程\
電子が余るn型と不足するp型を作り分けることでトランジスタが動く\
加速したイオンビームを照射する
...
### 3工程でトランジスタができる（断面イメージ）

成膜でゲート材を積み エッチングで形を作り イオン注入でn型/p型を作り込む

:::{image} _static/images/frontend-3step-pn-buildup.svg
:alt: 成膜・エッチング・イオン注入の3工程で p型基板にn型領域とゲートを持つトランジスタが立体的に形成される
:width: 80%
:align: center
:::

[出典: 断面は一般的なMOSトランジスタ工程の自作図/2025年頃\
詳細は docs「前工程」ページ]{.source}
...
各工程の後にパターン寸法・欠陥・膜厚などを測定し、歩留まり（作ったチップのうち良品の割合）を支える工程\
微細化が進むほど計測精度の重要性が増す
...
### トランジスタ構造の進化（平面→FinFET→GAA）

微細化でチャネルが短くなると オフでも電流が漏れる（リーク）\
ゲートがチャネルを囲む面を増やすほど オンオフの制御が効く
...
だから2nm世代の競争はGAAへ移る（TSMC/Samsung/Intel/[Rapidus]{.jp}）

[出典: 構造ロードマップは概況/2025年頃\
詳細は docs「前工程」ページ]{.source}

### ゲートがチャネルを囲む面（断面イメージ）

囲む面が多いほどオフでしっかり電流を止められる（リークを抑える）

:::{image} _static/images/transistor-gate-planar-finfet-gaa.svg
:alt: 平面・FinFET・GAAでゲートがチャネルを囲む面の比較
:width: 82%
:align: center
:::

[出典: 断面は一般的な構造の自作図/2025年頃\
詳細は docs「前工程」ページ]{.source}
